		...
		try {
			if (fileName.contains("..")) {
				throw new FileStorageException(String.format(
						MessageConst.ERROR_FILE_NAME_INVALID, fileName));
			}
			...
			throw new FileStorageException(String.format(
					MessageConst.ERROR_FILE_NOT_STORED, fileName), e);
		}
	}
	...
	public Resource loadFileAsResource(String fileName) {
		try {
			...
			if (resource.exists()) {
				...
				throw new MyFileNotFoundException(String.format(
						MessageConst.ERROR_FILE_NOTFOUND, fileName));
			}
		} catch (MalformedURLException ex) {
			throw new MyFileNotFoundException(String.format(
					MessageConst.ERROR_FILE_NOTFOUND, fileName), ex);
		}
	}

	public void deleteFile(String avatar) {
		File files = new File(getFileStorageLocation() + "\\" + avatar);
		files.delete();
	}
